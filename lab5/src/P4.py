...
class Relu(Layer):
    ...
    def backward(self, grad_output):
        ...
        grad_input = grad_output * \
            np.where(self._saved_tensor > 0, 1, 0)
        return grad_input
...
class Linear(Layer):
    ...
    def update(self, config):
        ...
        """Your code here"""
        self.diff_W = mm * self.diff_W + self.grad_W
        self.diff_b = mm * self.diff_b + self.grad_b

        self.W -= self.diff_W * lr
        self.b -= self.diff_b * lr
...
class Network(object):
    ...
    def backward(self, grad_output):
        ...
        for i in range(1, self.num_layers + 1):
            grad_output = self.layer_list[-i].backward(grad_output)
...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    for iter_idx, (input, label) in enumerate(data_iterator(inputs, labels, batch_size)):
        ...
        if iter_idx % disp_freq == 0:
            msg = '  Training iter %d, batch loss %.6f, batch acc %.6f' % (
                iter_idx, np.mean(loss_list), np.mean(acc_list))
