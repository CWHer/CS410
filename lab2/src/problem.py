...
class SearchProblem:
    ...
    def get_goal(self):
        return self._layout.goal

    def get_successors(self, state, wall=0):
        """
        state: current state

        wall:  0 for getting the successors of blank spaces

               1 for getting the successors of walls (character %)

               2 for getting the successors of all the adjacent states
        ...
        if wall == 0:
            ...
            for action in [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST]:
                ...
                dx, dy = Actions.direction_to_vector(action)
                next_x, next_y = int(x + dx), int(y + dy)
                if (next_x<0 or next_x>=self._layout._width) or (next_y<0 or next_y>=self._layout._height):
                    continue
                if self._layout.is_wall((next_x, next_y)):
                    next_state = (next_x, next_y)
                    cost = self._cost_fn(next_state)
                    successors.append((next_state, action, cost))
